<div class="inline update-inline-actions">
...
@if($available)
    <div class="panel update-release-card">
        <strong>Доступен релиз:</strong>
        <span class="mono">v{{ $available['version'] ?? 'unknown' }}</span>
        <div class="muted update-release-meta">
            Канал: <span class="mono">{{ $available['channel'] ?? 'n/a' }}</span>
        </div>
        @if(!empty($available['changelog_url']))
            <div class="update-release-meta">
                <a href="{{ $available['changelog_url'] }}" target="_blank" rel="noreferrer">Список изменений</a>
            </div>
        @endif
    </div>
@endif
...
@if($pending)
    <div class="panel update-release-card">
        ...
        <div class="muted update-release-meta">Источник: <span class="mono">{{ $pending['source'] ?? 'unknown' }}</span></div>
        @if(!empty($pending['zip_path']))
            <div class="muted update-release-meta">Файл: <span class="mono">{{ $pending['zip_path'] }}</span></div>
        @endif
    </div>
@endif
...
<h3 class="panel-heading">Ручной пакет (ZIP)</h3>
